  ...
  return (
    <div className={`flex items-center select-none ${isHorizontal ? 'flex-row w-full py-1' : 'flex-col gap-1 py-3 px-2 bg-white border-r border-gray-100 shadow-sm'}`}>
      {TOOLS.map((tool) => {
        ...
        return (
          <div key={tool.id} className={isHorizontal ? 'flex-1 flex flex-col items-center' : 'flex flex-col gap-1'}>
            <button
              onClick={() => onToolChange(tool.id)}
              title={`${label} (${tool.shortcut})`}
              className={`
                relative group ${isHorizontal ? 'flex-1 aspect-square w-full' : 'w-11 h-11 shrink-0'} rounded-xl flex items-center justify-center transition-all duration-150
            ...
            {/* Calibration mode: line vs area */}
            {isCalibrate && showCalibrationMode && (
